...
void MprpcChannel::CallMethod(const google::protobuf::MethodDescriptor* method, google::protobuf::RpcController* controller, const google::protobuf::Message* request, google::protobuf::Message* response, google::protobuf::Closure* done)
{
    ...
    */
    ...
    {
        char errtxt[512] = {0};
        sprintf(errtxt, "socket errno: %d", errno);
    ...
    if(host_data == "")
    {
        controller->SetFailed(method_path+"do not have");
        return;
    }
    int idx = host_data.find(":");
    if(idx==-1)
    {
        controller->SetFailed(method_path+"address is wrong");
        return;
    }
    ...
    if(!response->ParseFromArray(recv_buf, recv_size))
    {
        close(clientfd);
        char errtxt[512] = {0};
        sprintf(errtxt, "parse errno: %d", errno);
        controller->SetFailed(errtxt);
        return;
    }
    /*
    ...
    {
        std::cout<< "parse errno" << errno<< std::endl;
        close(clientfd);
        return ;
    }
    */
